 ...
 * @return
 */
float KameleonInterpolator::compute_pram(const std::string& variable, const float& c0, const float& c1, const float& c2,
...
//	std::cout << "KameleonInterpolator::compute_pram" << std::endl;
    ...
    n   =  interpolate(n_, c0, c1, c2,
				dc0, dc1, dc2);
//std::cout << "after interpolating n" << std::endl;
	string component0, component1, component2;
	if (modelName == mas_)
	{
		v0 =  interpolate("ur", c0, c1, c2,
						 dc0, dc1, dc2);
		if (v0 == missingValue)
			return missingValue;

		v1 =  interpolate("uphi", c0, c1, c2,
						 dc0, dc1, dc2);
		if (v1 == missingValue)
			return missingValue;

		v2 =  interpolate("utheta", c0, c1, c2,
						 dc0, dc1, dc2);
		if (v2 == missingValue)
			return missingValue;
	} else
	{
		v0 =  interpolate(ux_, c0, c1, c2,
				 dc0, dc1, dc2);
		if (v0 == missingValue)
			return missingValue;

		v1 =  interpolate(uy_, c0, c1, c2,
				 dc0, dc1, dc2);
		if (v1 == missingValue)
			return missingValue;

		v2 =  interpolate(uz_, c0, c1, c2,
				 dc0, dc1, dc2);
		if (v2 == missingValue)
			return missingValue;
...
{
	float dc0, dc1, dc2;
	return compute_pram(variable, c0, c1, c2,
			dc0, dc1, dc2);
}
