 ...
 *                      variables etc from the msfc_tvm cdf file.  This routine*
 ...
 *       HISTORY:                                                              *
 *                                                                             *
 *       12.07.2006     Maddox, Marlo    Initial development started           *
 *                                                                             *
 ******************************************************************************/
...
void get_msfc_tvm_cdf_info(
      int main_memory_flag)
{

   void allocate_defaults_msfc_tvm(
         void);
   ...
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, b_l_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, b_l_name, &b_l_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, b_t_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, b_t_name, &b_t_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, raz_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, raz_name, &raz_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   ...
   /* bats NX was float but ucla, ctip, enlil, msfc_tvm... are interger - no conversion needed here*/
   ...
   if ( DEBUG_FLAG)
   {
      printf("DEBUG\tINFO from get_msfc_tvm_cdf_info\n");
      printf(
            "DEBUG\tNX=%d & nx=%d\n",
            NX,
            nx);
...
int load_msfc_tvm_cdf_variable_into_main_memory(
      char *variable_to_read)
{
   ...
   int msfc_tvm_reserve_mem_and_set_cdf_num(
         char *);

   /*printf("DEBUG\tinside load\n");*/

   variable_cdf_number = msfc_tvm_reserve_mem_and_set_cdf_num(variable_to_read);
      ...
      void)
{
   ...
   {
      printf(
            "\nallocate_defaults_msfc_tvm calloc failed for %s buffer!\n",
            b_l_name);
      exit( EXIT_FAILURE);
   }
   if ( (b_t = ( float * ) calloc(
