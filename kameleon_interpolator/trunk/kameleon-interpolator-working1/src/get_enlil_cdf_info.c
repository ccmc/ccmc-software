 ...
 *                      Added r, phi, theta based variable names in            *
 *                      the  enlil_cdf_interface_variables.h header file.  Get *
 *                      info will now pull the correct variable numbers etc for*
 *                      both variable name styles x,y,z & r, phi, theta        *
 ******************************************************************************/
      ...
      int main_memory_flag)
{
   ...
   /***************** SINCE WE CHANGED TO A NEW R, PHI, THETA SCHEME, ALLOW
    * ORIGINAL NAMES TO VALIDATE BUT ALSO CHECK FOR NEW NAMES *****************/

   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, r_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, r_name, &x_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, phi_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, phi_name, &y_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, theta_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, theta_name, &z_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, T_name, NULL_) == CDF_OK)
   ...
   {
      status = CDFlib( GET_, zVAR_NUMBER_, br_name, &bx_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, bphi_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, bphi_name, &by_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, btheta_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, btheta_name, &bz_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, b1r_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, b1r_name, &b1x_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, b1phi_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, b1phi_name, &b1y_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, b1theta_name, NULL_) == CDF_OK)
   {
      status
            = CDFlib( GET_, zVAR_NUMBER_, b1theta_name, &b1z_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, ur_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, ur_name, &ux_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, uphi_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, uphi_name, &uy_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, utheta_name, NULL_) == CDF_OK)
   {
      status = CDFlib( GET_, zVAR_NUMBER_, utheta_name, &uz_cdf_varNum, NULL_);
      if (status != CDF_OK)
         StatusHandler(status);
   }
   if (CDFlib( CONFIRM_, zVAR_EXISTENCE_, rho_name, NULL_) == CDF_OK)
...
__FILE__      , variable_to_read );
      ...
      if ( ! strcmp( variable_to_read, bx_name ) || ! strcmp( variable_to_read, br_name ) )
      {
      ...
      else if ( ! strcmp( variable_to_read, by_name ) || ! strcmp( variable_to_read, bphi_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, bz_name ) || ! strcmp( variable_to_read, btheta_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, b1x_name ) || ! strcmp( variable_to_read, b1r_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, b1y_name ) || ! strcmp( variable_to_read, b1phi_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, b1z_name ) || ! strcmp( variable_to_read, b1theta_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, ux_name ) || ! strcmp( variable_to_read, ur_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, uy_name ) || ! strcmp( variable_to_read, uphi_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
      ...
      else if ( ! strcmp( variable_to_read, uz_name ) || ! strcmp( variable_to_read, utheta_name ))
      {
         status = CDFlib (
         SELECT_, zVAR_, variable_cdf_number,
         SELECT_, zVAR_RECNUMBER_, recordStart,
