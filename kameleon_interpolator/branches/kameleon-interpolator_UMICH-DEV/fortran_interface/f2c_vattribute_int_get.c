 ...
 *                   specific variable Attribute values of type int from       *
      ...
      int *vattr_int_value)
{
   ...
   vattr_int_valuePtr = vattribute_get(
         variable_name_copy_local_storage,
         vattr_name_copy);

   if (vattr_int_valuePtr == NULL)
   {
      printf(
            "ERROR aquiring attribute value for --> %s <--. setting value "
            "pointer = NULL & returning 0\n",
            vattr_name_copy);
      vattr_int_value = NULL;
      return 0;
   }

   vattr_int_value[0] = *vattr_int_valuePtr;

   /*
    printf("f2c_vattr_int_get.c\nDEBUG:\tvattr_int_value = ->%d<-\n", *vattr_int_value );
    printf("f2c_vattr_int_get.c\nDEBUG:\tvattr_int_valuePtr = ->%d<-\n", *vattr_int_valuePtr );
    */
