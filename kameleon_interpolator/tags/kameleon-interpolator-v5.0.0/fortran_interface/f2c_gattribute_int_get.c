 ...
 *                Attribute values of type int from ccmc standardized cdf file.*
 *                                                                             *
      ...
      int *gattr_int_value)
{
   ...
   int *gattr_int_valuePtr;

   gattr_name_copy = f2c_string_convert(gattr_name);

   gattr_int_valuePtr = gattribute_get(gattr_name_copy);

   if (gattr_int_valuePtr == NULL)
   {
      printf(
            "ERROR aquiring attribute value for --> %s <--. setting value "
            "pointer = NULL & returning 0\n",
            gattr_name_copy);
      gattr_int_valuePtr = NULL;
      return 0;
   }

   gattr_int_value[0] = *gattr_int_valuePtr;

   /*
    printf("f2c_gattr_int_get.c\nDEBUG:\tgattr_int_value = ->%d<-\n", *gattr_int_value );
    printf("f2c_gattr_int_get.c\nDEBUG:\tgattr_int_valuePtr = ->%d<-\n", *gattr_int_valuePtr );
    */
