...
int main(int argc, char *argv[])
{
    ...
    if (argc != 8)
    {
        printf("ERROR:this example program requires 7 arguments - (1)full path "
              "and file name of data file (2) VARIABLE NAME (3) x1 position (4) "
              "y1 position (5) z1 position (6) delta x (7) x2 \n");
        exit( EXIT_FAILURE );
    }
    ...
    for (n = X; n <= x2; n+=sample_factor)
    {
        ...
        interp_value1 = interpolate_batsrus_cdf(variable, n, Y, Z, 1, 0);
        printf("\t%s [ %f, %f, %f ]\t%g %s\t\n", variable, n, Y, Z,
