...
int read_record_5(verbose_flag)
    ...
    if ( !NX_set && !batsrus_special_parameter_NX_exists_in_databaseinfo)
        ...
        "located.  Set keyvalue in DatabaseInfo file to proceed...\nie. 6 "
...
int read_record_6(verbose_flag)
    ...
    if ( (x_position_arrayPtr = calloc(number_of_cells, sizeof(float)) )
        == NULL)
    {
    printf("MALLOC for x_position array FAILED\n");
    exit(EXIT_FAILURE);
    }
    if ( (y_position_arrayPtr = calloc(number_of_cells, sizeof(float)) )
        == NULL)
    {
    printf("MALLOC for y_position array FAILED\n");
    exit(EXIT_FAILURE);
    }
    if ( (z_position_arrayPtr = calloc(number_of_cells, sizeof(float)) )
        == NULL)
    {
...
int build_grid_description(verbose_flag)
    ...
    * block in *_position_array */
    ...
    for (j = 0; j < elements_per_block; j++)
        {
        if (block_x_min[ block_count ]
            > x_position_arrayPtr[ element_count ])
        {
        block_x_min[ block_count ]
            = x_position_arrayPtr[ element_count ];
        }
        if (block_x_max[ block_count ]
            < x_position_arrayPtr[ element_count ])
        {
        block_x_max[ block_count ]
            = x_position_arrayPtr[ element_count ];
        }
        if (block_y_min[ block_count ]
            > y_position_arrayPtr[ element_count ])
        {
        block_y_min[ block_count ]
            = y_position_arrayPtr[ element_count ];
        }
        if (block_y_max[ block_count ]
            < y_position_arrayPtr[ element_count ])
        {
        block_y_max[ block_count ]
            = y_position_arrayPtr[ element_count ];
        }
        if (block_z_min[ block_count ]
            > z_position_arrayPtr[ element_count ])
        {
        block_z_min[ block_count ]
            = z_position_arrayPtr[ element_count ];
        }
        if (block_z_max[ block_count ]
            < z_position_arrayPtr[ element_count ])
        {
        block_z_max[ block_count ]
            = z_position_arrayPtr[ element_count ];
        }
    ...
    for (ib=0; ib<number_of_blocks; ib++)
        ...
        p1_tmp=(global_x_max-global_x_min)
            /(block_x_max[ib]-block_x_min[ib]);
        p2_tmp=(global_y_max-global_y_min)
            /(block_y_max[ib]-block_y_min[ib]);
        p3_tmp=(global_z_max-global_z_min)
            /(block_z_max[ib]-block_z_min[ib]);
        if (p1_tmp < p1)
        {
        p1=p1_tmp;
        }
