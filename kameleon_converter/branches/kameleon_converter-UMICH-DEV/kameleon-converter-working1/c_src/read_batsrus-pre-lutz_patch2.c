...
int setup_parent(int iblock,int ilev,
  ...
  while (jlev < ilev){
	...
	return(-2);
      ...
      block_at_amr_level[jlev*N_blks+number_of_parent_blocks_at_amr_level[jlev]]=ichild;
      number_of_parent_blocks_at_amr_level[jlev]=number_of_parent_blocks_at_amr_level[jlev]+1;
...
int unique(float *input_array,int input_array_len,int stride,
  ...
  if ( (*output_index_array=(int*)malloc(sizeof(int)*((stride > 1)+(input_array_len/stride)))) == NULL ){
    fprintf(stderr,"%s: memory allocation failed\n",__FILE__);
    return(-1);
  }
  output_index_array[0]=0;
  test_index=1;
  while (test_index < input_array_len) {
    if (input_array[test_index] != input_array[n_unique-1]){
      (*output_index_array)[n_unique]=test_index;
      n_unique++;
    }
    test_index+=stride;
  }
...
int read_batsrus(char *input_file_name, int verbose_flag, int minmax_flag)
{
   ...
   if (special_parameter_NX <= 1) {
     ...
     while (x_position_arrayPtr[0] != x_position_arrayPtr[icell]){
       if (icell > number_of_cells/4) {return(1);} else {icell++;}
     }
   ...
   if (special_parameter_NY <= 1) {
     ...
     while (y_position_arrayPtr[0] != y_position_arrayPtr[icell]){
       if (icell > number_of_cells/(2*special_parameter_NX)) {return(1);} else {icell+=floor(special_parameter_NX);}
     }
   ...
   if (special_parameter_NZ <= 1) {
     ...
     n_uniq=unique(z_position_arrayPtr,number_of_cells,nv,&uniq_z);
     special_parameter_NZ=nzb;
     for (i_uniq=0;i_uniq<n_uniq-2;i_uniq++){
   ...
   if (verbose_flag)
   {  printf("Read_batsrus after read: Special parameters: NX: %f NY: %f NZ: %f\n", 
...
int read_record_5(verbose_flag)
{
         ...
         <number_special_parameters; i++, i2++)
   {
	  ...
	  strcmp(tmp_strarr.sarr[i2], "rbody") == 0)
...
int build_grid_description(verbose_flag)
{
   ...
   for (current_amr_level = 0,level_factor=1.;
	...
	/* setup virtual parent blocks */
	 ...
	 number_of_parent_blocks_at_amr_level[current_amr_level]=num_new_blocks;
...
#ifdef DEBUG
	   if (current_amr_level == 7 && i < 10)  fprintf(stderr,"current_amr_level: %i i: %i current_amr_level*N_blks+i: %i N_blks+number_of_parents+i: %i\n",current_amr_level,i,current_amr_level*number_of_blocks+i,number_of_blocks+number_of_parents+i);
#endif
	   block_at_amr_level[current_amr_level*number_of_blocks+i]=number_of_blocks+number_of_parents+i;
	 }
	 ...
	 for (iblk=0;iblk<number_of_blocks_at_amr_level[current_amr_level];iblk++){
	   ...
	   parent_block=setup_parent(ib,current_amr_level-1,
				     global_x_min,global_y_min,global_z_min,
				     number_of_blocks,number_of_blocks/7,&number_of_parents);
	   if (parent_block < 0){
	     /* error encountered */
	     exit (1);
	   }
