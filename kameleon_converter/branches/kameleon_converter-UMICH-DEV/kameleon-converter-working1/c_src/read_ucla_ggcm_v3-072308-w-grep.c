...
/* gridz.txt: z, z_bz1 */
   ...
   sprintf(sys_cmd_str,"grep -A 1 FIELD-3D-1 %s | grep -v FIELD-3D-1 | grep -v \"\\-\\-\" > ./tmp_varlist 2>/dev/null",ucla_data_file);
   if (verbose_flag){
     fprintf(stderr,"read_ucla_ggcm_v3 - executing: system(%s)\n",sys_cmd_str);
   }
   system(sys_cmd_str);

   /* set up default varlist */
   for (i_var=0;i_var<varlist_len;i_var++) {
     varlist[i_var]=(char*)realloc((void*)varlist[i_var],varlist_str_len);
   }
   strcpy(varlist[0],"bx");
   strcpy(varlist[1],"by");
   strcpy(varlist[2],"bz");
   ...
   strcpy(varlist[6],"rr");
   strcpy(varlist[7],"pp");
   strcpy(varlist[8],"resis");
   strcpy(varlist[9],"xjx");
   strcpy(varlist[10],"xjy");
   strcpy(varlist[11],"xjz");
   strcpy(varlist[12],"bx1");
   strcpy(varlist[13],"by1");
   strcpy(varlist[14],"bz1");
   number_of_variables_in_data=15;

   if ( (TMPFILE=fopen("./tmp_varlist","r") ) != NULL ) {
     i_var=0;
     while (fscanf(TMPFILE,"%s ",tmp_string) == 1){
       strcpy(varlist[i_var],tmp_string);
       fprintf(stderr,"Varlist[%i]: %s\n",i_var,varlist[i_var]);
       i_var++;
     }
     if (i_var > 0 && (strcmp(varlist[i_var-1],"") != 0) ) {
...
int allocate_space_for_data(int dimension_size)
{
   ...
   if ( (resistivity_resis = calloc(dimension_size, sizeof(float)) ) == NULL)
   {
      printf(
...
int read_3df_data(int verbose_flag, char *ucla_data_file)
{
 ...
 for (i_var=0;i_var < number_of_variables_in_data;i_var++){
   ...
   if (verbose_flag)
   {
      printf("reading from varlist %s...\n", varlist[i_var]);
   }


  magnetic_field_bx_name[7] = '\0';
  if (strcmp(varlist[i_var],magnetic_field_bx_name) == 0){
