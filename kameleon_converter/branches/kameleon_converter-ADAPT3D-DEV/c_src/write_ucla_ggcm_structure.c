      ...
      int write_database_info_flag)
{
   ...
   strcpy(
         input_file_name_local_copy,
         input_file_name);
   input_file_name_local_copy[ file_name_length] = '\0';

   strcpy(
         output_directory_copy,
         output_directory);
   strcpy(
         converted_file_name,
         input_file_name_local_copy);
   ...
   if ( DEBUG_FLAG)
   {
      ...
      printf(
            "\n\nDEBUG\tsimulation init time = %s\n",
            open_ggcm_init_time_epoch3);
      printf(
            "DEBUG\tsimulation start time = %s\n",
            start_time_cdf_epoch3);

      printf(
            "DEBUG\nsimulation start time using parseEPOCH3 = %f\n",
            parseEPOCH3(start_time_cdf_epoch3) );
      printf(
            "DEBUG\nsimulation init time using parseEPOCH3 = %f\n",
            parseEPOCH3(open_ggcm_init_time_epoch3) );

      printf("assigning to exp1 and exp2...\n");
   }
   ...
   if ( DEBUG_FLAG)
   {
      ...
      printf(
            "MMMDEBUG\tstart_time_delta = ( %f - %f ) = %f\n",
            exp1,
            exp2,
            start_time_delta);
   }
   ...
   if ( DEBUG_FLAG)
   {
      ...
      printf(
            "MMMDEBUG\tstart_time_delta_float = ( %f * %f ) = %f\n",
            (float) start_time_delta,
            operand1,
   ...
   if ( DEBUG_FLAG)
   {
      ...
      printf(
            "MMMDEBUG\telapsed_time_in_seconds = %f - %f = %f\n",
            extracted_file_name_time_value,
            start_time_delta_float,
            elapsed_time_in_seconds);

      printf(
            "DEBUG\textracted_file_name_time_value = %f\nstart_time_delta = %f\n",
            extracted_file_name_time_value,
            start_time_delta);
   }
   ...
   /****************************************************************************
    ...
    parseEPOCH3( open_ggcm_init_time_epoch3 ) ) ) / 1000.0;
      ...
      BUG UPDATE - Tracked to read_database.c & date_formatter...null termination
      erros for open_ggcm Database Info string processing - fixed 08152008 -
      M. Maddox
