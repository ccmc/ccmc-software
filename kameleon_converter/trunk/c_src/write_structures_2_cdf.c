      ...
      char *original_working_directory)
{
   ...
   int set_cdf_attr_arguments(
         int);
   int set_cdf_var_arguments(
         int);
   int set_cdf_var_attr_arguments(
         int,
         int);
      ...
      if ( !status2) /** set_cdf_var_arguments returned successfully **/
      {
         ...
         if (current_dimSizes[0] > 0)
         {
            ...
            for (j = 0; j < NUMBER_CCMC_VARIABLE_ATTR; j++)
            {
               ...
               status2 = set_cdf_var_attr_arguments(
                     i,
                     j);
      ...
      int struct_number)
{
         ...
         "char") )
   {
      ...
      if (current_numElements > 0)
         return EXIT_SUCCESS; /** if the stringlen <= 0 there is no need to try to write value to CDF as this will confuse the CDFattrPut call if numElements is zero **/
      else
         return EXIT_FAILURE;
   }
   else if ( !strcmp(
