 ...
 *                   this is a bug iin which gridx[0] or bx[0] was being       *
 ...
 *                   so subsequent f77_file_opens were not recieving the       *
 *                   correct arguments for formating thus the improper values  *
...
int read_stagered_grids(int, char *);
      ...
      int minmax_flag)
{
   ...
   int read_stagered_grid(int, char *);
   ...
   if ( !local_call_count)
   {
      ...
      /**** validate grid and 3df file before proceding, if grid file is bad we
      ...
      /******** read stagered grid positions: gx-bx, gy-bx, gz-bx, gx-by, gy-by,
       * gz-by, gx-bz, gy-bz, gz-bz using fortran getf11() *************/

      return_status = read_stagered_grid(verbose_flag, ucla_grid_file);

      if (return_status)
      {
         if (verbose_flag)
         {
            printf("\nstagered grid positions [ gx-bx, gy-bx, gz-bx ] , "
      ...
      {
         /**** ERROR HANDLER *******/
         printf("ERROR:\tread_stagered_grid FAILED !\n");
      }
      ...
      int verbose_flag)
{
   ...
   {
      fprintf( stderr, "ERROR openeing grid file: %s \nEXITING "
   ...
   {
      fprintf( stderr, "ERROR openeing data file: %s \nEXITING "
...
int read_standard_grid(int verbose_flag, char *ucla_grid_file)
{
   ...
   if (nx == -1)
   {
...
#el
            se f77_file_rewind_( &f77_open_file_unit_n, &return_status );
#endif
      ...
      getf11_( &f77_open_file_unit_n, &gridx[0], &nx, &gridx_name[0],
            &l2[0], &it, &five_char );
      if( nx != -1 ) printf("retry succesful.\n");
   }
   ...
   if( ny == -1 )
   {
      ...
      getf11_( &f77_open_file_unit_n, &gridy[0], &ny, &gridy_name[0], &l2[0],
            &it, &five_char );
      if( ny != -1 ) printf("retry succesful.\n");
   }
   ...
   if( nz == -1 )
   {
      ...
      getf11_( &f77_open_file_unit_n, &gridz[0], &nz, &gridz_name[0], &l2[0],
            &it, &five_char );
      if( nz != -1 ) printf("retry succesful.\n");
   }
...
int read_stagered_grid(int verbose_flag, char *ucla_grid_file)
{
   int return_code = 1;
   ...
   if (nx == -1)
   {
      ...
      __FILE__ );
            ...
            f77_file_rewind__( &f77_open_file_unit_n, &return_status );
#els
            e f77_file_rewind_( &f77_open_file_unit_n, &return_status );
#endif
...
if      ( return_status )
      ...
      if( nx >= 0 && ny >= 0 && nz >= 0 ) printf("retry succesful.\n");
   }
