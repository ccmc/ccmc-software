 ...
 *                   every getf* call for standard/staggered grid and data     *
 *                   testing to see if nan's disappear.  Fixed gridx[0] nan's  *
 *                   by restricting grid read to once per excecution.  This    *
 *                   is more efficient anyway since the grid files are static  *
 *                   The nan issue arises when multiple input files are        *
 ...
 *                   precision.                                                *
 *                                                                             *
 *    07.15.2008     Rastaetter, Lutz                                          *
 *                   Added pre-processing to aquire variables names            *
 *                                                                             *
 *                   ...also added ccmc_missing.h include                      *
 *                   ...also pre-popluate arrays with missing values           *
 *                                                                             *
 *    07.25.2008     Maddox, Marlo                                             *
 *                   Addition from 6.10.2008 uses hardcoded file names and     *
 *                   assumes executable is run from the data directory and     *
 *                   tries to open grid*.txt.  Adding functionality to         *
 *                   dynamically build full path to optional grid files using  *
 *                   data file name and string processing.                     *
 *                                                                             *
 *                   Fixed bug that put required variables:                    *
 *                      f77_open_file_status_len                               *
 *                      f77_open_file_format_len                               *
 *                   inside new grid functionality block that is only executed *
 *                   conditionaly whereas they need to allways be set for all  *
 *                   fortran reads.                                            *
 ******************************************************************************/
...
int read_standard_grid(
      int,
      char *);
int read_staggered_grids(
      int,
      char *);
int allocate_space_for_data(
      int);
int read_3df_data(
      int,
      char *);
...
extern void f77_file_open__(
      ...
      int *,
      int *);
extern void f77_file_rewind__(
      int *,
      int *);
extern void f77_file_close__(
      int *,
      int *);
#else
...
/* issue system command and read list of variables from a short text file */
FILE *TMPFILE;
int i_var=0;
#define varlist_len 30 /* length of variable list expeced in data */
#define varlist_str_len 20 /* length of longest variable name minus 1 */

char sys_cmd_str[200]; /* system command to obtain list */
char tmp_string[varlist_str_len]; /* single string to reas in a line */
char *varlist[varlist_len]; /* array of char pointers to hold names */
int number_of_variables_in_data=0; /* will have the number of variables read */
      ...
      int minmax_flag)
{
   ...
   int system_command_return_status;
   ...
   /** MM - Adding dynamic full path and filename for grid files **/

   char *grid_x_name = "gridx.txt";
   char *grid_y_name = "gridy.txt";
   char *grid_z_name = "gridz.txt";

   char gridx_file[MAX_FILE_NAME_LEN];
   char gridy_file[MAX_FILE_NAME_LEN];
   char gridz_file[MAX_FILE_NAME_LEN];

   char *end_of_path_pointer;
   char backslash = '/';

   /* Try to dynamically build the full path and file names for the optional
    * grid files.
    */

   end_of_path_pointer = strrchr(
         ucla_data_file,
         backslash);

   if (end_of_path_pointer != NULL)
   {
      /** code was NOT invoked in same directory with data
       *  extract pathname from ucla_data_file string **/

      int full_pathname_len = strlen(ucla_data_file);
      int basename_len = strlen(end_of_path_pointer);
      int new_base_location;

      new_base_location = full_pathname_len - basename_len + 1;

      /** new_base_location gives us the location in the path to replace the
       *  data file name with our grid names **/

      if (new_base_location > 0)
      {
         strncpy(
               gridx_file,
               ucla_data_file,
         ...
         gridx_file[new_base_location] = '\0';
         gridy_file[new_base_location] = '\0';
         gridz_file[new_base_location] = '\0';
         ...
         if (verbose_flag)
         {
            printf("Dynamically building full paths for alternative grid files\n");
            printf(
                  "%s\n",
                  gridx_file);
            printf(
      ...
      else /** we are either in the root or the actuall data directory **/
      {
         strncpy(
               gridx_file,
               grid_x_name,
   ...
   {
      printf(
            "ERROR Openeing %s...reverting to default grid file\n",
            gridx_file);
      use_gridx=0;
   ...
   if (use_gridx)
   {
      ...
      for (i=0; i<MAX_GRID_SIZE_X; i++)
      {
         ...
         if (fscanf(
               ...
               &x3) !=3)
         {
            /*fprintf(stderr,"End of file reached: %s %i\n", gridx_file,i);*/
            break;
         }
         gridx[i]=x1;
         gridx_bx[i]=x3;
            ...
            "r") ) == NULL)
      {
         printf(
               "ERROR Openeing %s...reverting to default grid file\n",
               gridy_file);
         /* exit(1); *//** MM Probably Should Die Here **/
      }
      else
      {
         if (verbose_flag)
...
__FILE__         ,
      ...
      sprintf(
      sys_cmd_str,
      "grep -A 1 FIELD-3D-1 %s | grep -v FIELD-3D-1 | grep -v \"\\-\\-\" > ./tmp_varlist 2>/dev/null",
      ucla_data_file);

      if (verbose_flag)
      {
         /*fprintf(stderr,"read_ucla_ggcm_v3: Attempting To Execute System Command For Variable Order Pre-Processing: system( %s )...",sys_cmd_str);*/
         printf("%s: Attempting To Execute System Command For Variable Order Pre-Processing: system( %s )...",__FILE__,sys_cmd_str);
      }

      /** system command should return 0 on sucess **/

      system_command_return_status = system(sys_cmd_str);

      if (verbose_flag && system_command_return_status != 0)
      {
         printf("!!! Variable Order Pre-Processing FAILED !!!\n\nUsing Default Variable Order...\n");
      }

      printf("\n");

      /* set up default varlist */
      for (i_var=0; i_var<varlist_len; i_var++)
      {
         varlist[i_var]=(char*)realloc(
         (void*)varlist[i_var],
         varlist_str_len);
      }
      strcpy(
      ...
      varlist[14],
      "bz1");
      number_of_variables_in_data=15;
                  ...
                  "r") ) != NULL)
      {
         ...
         if (i_var> 0 && (strcmp(
                     ...
                     "") != 0))
         {
            /* update only if file wasn't empty */
            number_of_variables_in_data=i_var;
         }
         fclose(TMPFILE);
      }
      ...
      MAX_COPY_LEN);
      strncpy(
      f77_open_file_status_2,
      f77_open_file_status,
      MAX_STATUS_LEN);
      strncpy(
      f77_open_file_format_2,
      f77_open_file_format,
      MAX_STATUS_LEN);
      ...
      if (have_ucla_grid == 0)
      {
         ...
         if ( !local_call_count)
         {
            ...
            {
               printf(
               "ERROR\tfortran open statement failed for %s\n",
               ucla_grid_file);
            } /* ERROR HANDLER */

            /* read regular grid positions:gridx, gridy, gridz using fortran getf11() */

            return_status = read_standard_grid(
            verbose_flag,
            ucla_grid_file);
            ...
            return_status = read_staggered_grid(
            verbose_flag,
            ucla_grid_file);
      ...
      } /*** end of grid reading section - turned off if we read ascii files */
      ...
      int verbose_flag)
{
   ...
   if (strlen(ucla_grid_file) > 0)
   {
      ...
      else
      {
         ...
         /* strcmp breaks on empty strings and binary files with segmaentation
   ...
   {
      valid_grid_file = 1;
   } /* we didn't get a file to test */

   if (verbose_flag)
   {
      printf(
            "valid_grid_file flag being set = %d\n",
            valid_grid_file);
   }
   ...
   {
      printf(
            "valid_data_file flag being set = %d\n",
            valid_data_file);
   }
      ...
      char *ucla_grid_file)
{
   ...
   {
      printf(
            "%s\tWARNING: EOF or file read error encountered. Rewinding file "
               "and retrying...",
            __FILE__ );
      ...
      char *ucla_grid_file)
{
   ...
   gridx_bx_name[gridx_bx_name_len] = '\0';
...
if      ( return_status )
   ...
   gridy_bx_name[gridy_bx_name_len] = '\0';
   ...
   gridz_bx_name[gridz_bx_name_len] = '\0'; /* terminate string again, for some reason second
   ...
   gridx_by_name[gridx_by_name_len] = '\0';
   ...
   gridy_by_name[gridy_by_name_len] = '\0';
   ...
   /* terminate string again, for some reason second calls to string names
    * ( defined in read_ucla.h ) behave funny */
   gridz_by_name[gridz_by_name_len] = '\0';
   ...
   gridx_bz_name[gridx_bz_name_len] = '\0';
   ...
   gridy_bz_name[gridy_bz_name_len] = '\0';
   ...
   gridz_bz_name[gridz_bz_name_len] = '\0';
      ...
      int dimension_size)
{
   ...
   if ( (resistivity_resis = calloc(
         dimension_size,
         sizeof(float)) ) == NULL)
   {
      printf(
            "ERROR:\tCALLOC FAILED for resistivity_resis array of size %d\n",
            dimension_size);
      exit( EXIT_FAILURE);
   }
   /* set arrays to missing (mask) value */
   for (i=0; i<dimension_size; i++)
   {
      velocity_vx[i]=FLOAT_MASK;
      velocity_vy[i]=FLOAT_MASK;
      velocity_vz[i]=FLOAT_MASK;
      resistivity_resis[i] =FLOAT_MASK;
      magnetic_field_bx1[i]=FLOAT_MASK;
      magnetic_field_by1[i]=FLOAT_MASK;
      magnetic_field_bz1[i]=FLOAT_MASK;
      magnetic_field_bx[i]=FLOAT_MASK;
      magnetic_field_by[i]=FLOAT_MASK;
      magnetic_field_bz[i]=FLOAT_MASK;
      density_rr[i]=FLOAT_MASK;
      pressure_pp[i]=FLOAT_MASK;
      current_density_xjx[i]=FLOAT_MASK;
      current_density_xjy[i]=FLOAT_MASK;
      current_density_xjz[i]=FLOAT_MASK;
   }
      ...
      char *ucla_data_file)
{
   ...
   for (i_var=0; i_var < number_of_variables_in_data; i_var++)
   {

      if (verbose_flag)
      {
         printf(
               "reading from varlist %s...\n",
               varlist[i_var]);
      }

      printf("\nDEBUG\tvarlist[%d] = >%s<\n", i_var, varlist[i_var]);
      printf("DEBUG\tmagnetic_field_bx_name = >%s<\n", magnetic_field_bx_name);

     /* magnetic_field_bx_name[7] = '\0'; */
      magnetic_field_bx_name[magnetic_field_bx_name_len] = '\0';

      printf("DEBUG\tvarlist[%d] = >%s<\n", i_var, varlist[i_var]);
            printf("DEBUG\tmagnetic_field_bx_name = >%s<\n\n", magnetic_field_bx_name);
...
if            ( return_status )
      ...
      magnetic_field_by_name[magnetic_field_by_name_len] = '\0';

      if (strcmp(varlist[i_var],magnetic_field_by_name) == 0)
      {
      ...
      magnetic_field_bz_name[magnetic_field_bz_name_len] = '\0';

      if (strcmp(varlist[i_var],magnetic_field_bz_name) == 0)
      {
      ...
      velocity_vx_name[velocity_vx_name_len] = '\0';

      if (strcmp(varlist[i_var],velocity_vx_name) == 0)
      {
      ...
      velocity_vy_name[velocity_vy_name_len] = '\0';

      if (strcmp(varlist[i_var],velocity_vy_name) == 0)
      {
      ...
      velocity_vz_name[velocity_vz_name_len] = '\0'; /* terminate string again, for some reason
       second calls to string names ( defined in read_ucla.h ) behave funny */

      if (strcmp(varlist[i_var],velocity_vz_name) == 0)
      {
      ...
      density_rr_name[density_rr_name_len] = '\0';

      if (strcmp(varlist[i_var],density_rr_name) == 0)
      {
         if( verbose_flag )
         {  printf("reading %s...\n", density_rr_name );}
      ...
      pressure_pp_name[pressure_pp_name_len] = '\0';

      if (strcmp(varlist[i_var],pressure_pp_name) == 0)
      {
         if( verbose_flag )
         {  printf("reading %s...\n", pressure_pp_name );}
      ...
      magnetic_field_bx1_name[magnetic_field_bx1_name_len] = '\0';

      if (strcmp(varlist[i_var],magnetic_field_bx1_name) == 0)
      {
      ...
      magnetic_field_by1_name[magnetic_field_by1_name_len] = '\0';

      if (strcmp(varlist[i_var],magnetic_field_by1_name) == 0)
      {
         if( verbose_flag )
      ...
      magnetic_field_bz1_name[magnetic_field_bz1_name_len] = '\0';

      if (strcmp(varlist[i_var],magnetic_field_bz1_name) == 0)
      {
         if( verbose_flag )
      ...
      current_density_xjx_name[current_density_xjx_name_len] = '\0';

      if (strcmp(varlist[i_var],current_density_xjx_name) == 0)
      {
         if( verbose_flag )
      ...
      current_density_xjy_name[current_density_xjy_name_len] = '\0';
      if (strcmp(varlist[i_var],current_density_xjy_name) == 0)
      {
         if( verbose_flag )
      ...
      current_density_xjz_name[current_density_xjz_name_len] = '\0';
      if (strcmp(varlist[i_var],current_density_xjz_name) == 0)
      {
         if( verbose_flag )
      ...
      resistivity_resis_name[resistivity_resis_name_len] = '\0';
      if (strcmp(varlist[i_var],resistivity_resis_name) == 0)
      {
         if( verbose_flag )
         {  printf("reading %s...\n", resistivity_resis_name );}

         getf31_( &f77_open_file_unit_n, &resistivity_resis[0], &nx, &ny, &nz,
               &resistivity_resis_name[0], &l2[0], &it, &three_char );
         if( nx < 0 || ny < 0 || nz < 0 )
         {
            /* nx|ny|nz will be equal to -1 if there was a problem reading */
            return_code = -1;
