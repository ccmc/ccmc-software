...
/* gridz.txt: z, z_bz1 */
   ...
   sprintf(sys_cmd_str,"/bin/grep -A 1 FIELD-3D-1 %s | /bin/grep -v FIELD-3D-1 | /bin/grep -v \"\\-\\-\" > ./tmp_varlist 2>/dev/null",ucla_data_file);
   fprintf(stderr,"read_cla_ggcm_v3 - executing: system(%s)\n",sys_cmd_str);
   system(sys_cmd_str);
...
if      ( return_status )
  ...
  {
    fprintf(stderr,"By1??? %s %s %ld\n",varlist[i_var],magnetic_field_by1_name,strlen(varlist[i_var]) );
   if( verbose_flag )
