      ...
      int max_level)
{
   ...
   if (debug)
   {
      fprintf(stderr,"Climbing tree at root: %i \nXMIN: %f XMAX: %f X: %f IX: %i\nYMIN: %f YMAX: %f Y: %f IY: %i\nZMIN: %f ZMAX: %f Z: %f IZ: %i\n",
      root,
      block_x_min[root],block_x_max[root],x,ix,
      block_y_min[root],block_y_max[root],y,iy,
      block_z_min[root],block_z_max[root],z,iz
      ...
      int *N_parents)
{
   ...
   ixyz=floor(0.001+ix+special_parameter_P1*(iy+special_parameter_P2*iz));
   root=block_at_amr_level[ixyz];
   jlev=0;
   if (DEBUG >= 2)
   {
      fprintf(stderr,"Setup Parent: xc %f yc %f zc %f\np1: %f p2: %f p3: %f\n",xc,yc,zc,special_parameter_P1,special_parameter_P2,special_parameter_P3);
      fprintf(stderr,"Setup Parent: Root dx %f dy %f dz %f\n",dx_root,dy_root,dz_root);
   ...
   while (jlev < ilev)
   {
      ...
      if (ichild < 0 || ichild == root)
      {
         ...
         block_at_amr_level[jlev*N_blks+number_of_parent_blocks_at_amr_level[jlev]]
               =ichild;
         ...
         block_x_center[ichild]=(block_x_min[ichild]+block_x_max[ichild])/2.;
         block_y_center[ichild]=(block_y_min[ichild]+block_y_max[ichild])/2.;
         block_z_center[ichild]=(block_z_min[ichild]+block_z_max[ichild])/2.;
      ...
      int **output_index_array)
{
   ...
   if ( (*output_index_array=(int*)calloc((stride> 1)+(input_array_len/stride),sizeof(int) ) ) == NULL )
         {
      ...
      int minmax_flag)
{
   ...
   extern int linear_minmax_search(
         float *,
         int,
         float *,
         float *);
   ...
   special_parameter_NX,special_parameter_NY,special_parameter_NZ,
   special_parameter_P1,special_parameter_P2,special_parameter_P3);
      ...
      verbose_flag)
{
   ...
   if (verbose_flag)
      printf(
            "Read record_3: Flip_endian: %d Is double precision: %d Number "
               "elements: %d\n",
            flip_endian,
            is_double,
            number_elements);
      ...
      verbose_flag)
{
   ...
   if (verbose_flag)
      printf(
            "Read_record_4: number_elements: %d\n",
            number_elements);
      ...
      verbose_flag)
{
   ...
   {
      printf(
            "Variable name string array length: %ld\n",
            tmp_strarr.len);
      printf(
   ...
   for (
         i=0,
         i2=tmp_strarr.len-number_special_parameters; i
         <number_special_parameters;
         i++,
         i2++)
   {
   ...
   if (!th_set && batsrus_special_parameter_th_exists_in_databaseinfo)
   {
      ...
      else
      {
         ...
         int return_status=0;
         double cxform_et,cdf_epoch_start_time_value,cdf_epoch_dipole_time_value,cxform_et_offset_to_cdf_epoch;
         ...
         cxform_et_offset_to_cdf_epoch= computeEPOCH(2000,1,1,12,0,0,0);
         cxform_et=(cdf_epoch_dipole_time_value-cxform_et_offset_to_cdf_epoch)/1000.;
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter NX could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter NY could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter NZ could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter P1 could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter P2 could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   {
      fprintf( stderr, "!!!! %s\tWARNING: special parameter P3 could not be "
      "located.  Will attempt to compute parameter\n"
   ...
   if (verbose_flag)
   {  printf("Read Record 5: Special parameters: NX: %f R: %f P3: %f\n", special_parameter_NX,
      ...
      verbose_flag)
{
   ...
   if (verbose_flag)
   {
      ...
      printf(
            "number of original blocks =\t%d\n",
            number_of_blocks);
      printf(
            "original blocks plus virtual =\t%d\n",
            block_space);
   }
   ...
   if ( (block_x_center = (float *) calloc(
         block_space,
         sizeof(float)) ) == NULL)
   {
   ...
   if ( (block_y_center = (float *) calloc(
         block_space,
         sizeof(float)) ) == NULL)
   {
   ...
   if ( (block_z_center = (float *) calloc(
         block_space,
         sizeof(float)) ) == NULL)
   {
   ...
   if ( (block_amr_levels = (float *) calloc(
         block_space,
         sizeof(float)) ) == NULL)
   {
   ...
   {
      cell_delta_x[block_count] = x_position_arrayPtr[ ( block_count * NX * NY * NZ ) + 1 ] - x_position_arrayPtr[ ( block_count * NX * NY * NZ ) ];
      cell_delta_y[block_count] = y_position_arrayPtr[ ( block_count * NX * NY * NZ ) + NX ] - y_position_arrayPtr[ ( block_count * NX * NY * NZ ) ];
      cell_delta_z[block_count] = z_position_arrayPtr[ ( block_count * NX * NY * NZ ) + ( NX * NY ) ] - z_position_arrayPtr[ ( block_count * NX * NY * NZ ) ];
   }
   ...
   if (P1 <= 0 || P2 <= 0 || P3 <= 0)
   {
      ...
      p2=100000.;
      p3=100000.;
      fprintf(stderr,"Global xmin: %f xmax: %f\n",global_x_min,global_x_max);
      fprintf(stderr,"Global ymin: %f ymax: %f\n",global_y_min,global_y_max);
      fprintf(stderr,"Global zmin: %f zmax: %f\n",global_z_min,global_z_max);
      for (ib=0; ib<number_of_blocks; ib++)
      {
         float p1_tmp, p2_tmp, p3_tmp;
         p1_tmp=(global_x_max-global_x_min) /(block_x_max[ib] -block_x_min[ib]);
         p2_tmp=(global_y_max-global_y_min) /(block_y_max[ib] -block_y_min[ib]);
         p3_tmp=(global_z_max-global_z_min) /(block_z_max[ib] -block_z_min[ib]);
         if (p1_tmp < p1)
         {
            p1=p1_tmp;
         }
      ...
      while ( ( (p1/2-floor(p1/2+0.25)) < 0.4) && ( (p2/2-floor(p2/2+0.25))
            < 0.4) && ( (p3/2-floor(p3/2+0.25)) < 0.4))
      {
         p1/=2;
         p2/=2;
         p3/=2;
   ...
   for (
         current_amr_level = 0,
         level_factor=1.; current_amr_level <= max_amr_level - 1;
         current_amr_level++,
         level_factor =level_factor*2.)
   {
      if (number_of_blocks_at_amr_level[current_amr_level] <= 0)
      {
         float dx, dy, dz; /* block size at level current_amr_level */
         int iblk, N_parents_old, num_new_blocks, N_ix, N_iy, N_iz, ix, iy, iz;
            ...
            for (iy=0; iy<N_iy; iy++)
            {
               ...
               for (ix=0; ix<N_ix; ix++)
               {
                  ...
                  if (current_amr_level > 0)
                  {
                     ...
                     ip=block_at_amr_level[ number_of_blocks * ( current_amr_level - 1 ) + iparent ];
                     if (block_amr_levels[ip] != (current_amr_level-1))
                     {
                        ...
                        ix,N_ix,iy,N_iy,iz,N_iz,iparent,current_amr_level-1);
                        fprintf(stderr,"Setup Octree: parent block %i in wrong refinement level %f!=%i (current_amr_level).\n",ip,block_amr_levels[ip],current_amr_level);
                        exit(1);
                     }
                     if (parent_id[iblk] < 0)
                     {
                     ...
                     {
                        fprintf(stderr,"Block ip=%i:  Child block already assigned: iv_c=%i ix_c=%i iy_c=%i iz_c=%i\n",ip,iv_c,ix_c,iy_c,iz_c);
                     }
                  }
               }
            }
         ...
         number_of_parent_blocks_at_amr_level[current_amr_level]
               =num_new_blocks;
         fprintf(stderr,"Setup_octree: adding %i blocks to block_at_amrlevel\n %i\n",num_new_blocks,current_amr_level);
         for (i=0; i<=num_new_blocks-1; i++)
         {
#ifdef DEBUG
            if (current_amr_level == 7 && i < 10)
               fprintf(stderr,"current_amr_level: %i i: %i current_amr_level*N_blks+i: %i N_blks+number_of_parents+i: %i\n",current_amr_level,i,current_amr_level*number_of_blocks+i,number_of_blocks+number_of_parents+i);
#endif
      ...
      {
         int iblk;
         for (iblk=0; iblk<number_of_blocks_at_amr_level[current_amr_level]; iblk++)
         {
            int ib, parent_block, ichild, ix_c, iy_c, iz_c;
            float xc, yc, zc;
            ib=block_at_amr_level[current_amr_level*number_of_blocks+iblk];
            ...
            else
            {
               ...
               fprintf(stderr,"Ilev: %i Block: %i Parent: %i\n",current_amr_level,ib,parent_block);
               fprintf(stderr,"Parent XC: %f YC: %f ZC: %f\n",block_x_center[parent_block],
               block_y_center[parent_block],block_z_center[parent_block]);
               fprintf(stderr,"Block XC: %f YC: %f ZC: %f\n",block_x_center[ib],
               block_y_center[ib],block_z_center[ib]);
               fprintf(stderr,"IX: %i IY: %i IZ: %i\n",ix_c,iy_c,iz_c);
               fprintf(stderr,"Child IDs assigned: %i %i %i %i %i %i %i %i\nChiuld count: %i",
               ...
               child_id_8[parent_block],child_count[parent_block]);
               exit(1);
            }
         }
      }
      fprintf(stderr,"Setup_Octree::new_algorithm: level: %i of %f level_factor: %f\n",current_amr_level,max_amr_level-1,level_factor);
      fprintf(stderr," N_Parents: Total: %i Max: %i\n",number_of_parents,number_of_blocks/7);
   }
