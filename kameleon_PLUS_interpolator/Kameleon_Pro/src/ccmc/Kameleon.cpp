...
std::string Kameleon::getCurrentTime()
{
    ...
    if (start_time_char == "")
    {
        ...
        if (modelName == "enlil")
        {
            ...
            time_string = padString(time_string_year, 4) + "/" + padString(time_string_month,2) + "/" +  padString(time_string_day, 2) + " " +
				 padString(time_string_hours,2)+ ":" + padString(time_string_minutes, 2) + ":" + padString(time_string_seconds, 2);
		}
...
{
	boost::unordered_map<std::string, float>::iterator iter = conversionFactorsToVis.find(
				variable);
