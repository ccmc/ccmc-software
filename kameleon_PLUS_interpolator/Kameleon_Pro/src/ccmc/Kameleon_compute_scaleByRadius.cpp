 ...
 *
 */
float KameleonInterpolator::compute_scaleByRadius(const std::string& variable, const float& positionComponent1, const float& positionComponent2, const float& positionComponent3)
{
	float dComponent1, dComponent2, dComponent3;
	return compute_scaleByRadius(variable, positionComponent1, positionComponent2, positionComponent3,
		...
		float& dComponent1, float& dComponent2, float& dComponent3)
{
	...
	float interp_value = interpolate((std::string)baseVariable.c_str(), positionComponent1, positionComponent2, positionComponent3,
			dComponent1, dComponent2, dComponent3);
	if (interp_value == missingValue) return missingValue;
