...
public class CDFWriter extends Writer {
	...
	public void convert (Model m) throws NoAttributeException{
		...
		globalCDFattrs = new Attribute[m.GloAttrsSize()];
		...
		CDFvariables = new gsfc.nssdc.cdf.Variable[m.VarObjsSize()];


		try {
			//creates the new CDF file and sets some settings
			CDF cdf = CDF.create(newfilename);
			...
			for(int i=0; i<m.GloAttrsSize(); i++){
				...
				try {
					if(m.getGlobalAttribute(i).value!=null || m.getGlobalAttribute(i).value==""){
						...
						String dType = m.getGlobalAttribute(i).dataType.toString();
						...
						if(dType.equalsIgnoreCase("double"))
							...
							Entry.create(globalCDFattrs[i], 0, CDFConstants.CDF_CHAR, m.getGlobalAttribute(i).value);
			...
			for(int i=0; i<m.VarObjsSize(); i++){
				...
				for(int j=0; j<variableCDFattrs_size; j++){
					try {
						...
						String datatype = m.getVariableObject(i).getAttributeObject(variableCDFattrs[j].getName()).dataType.toString();
						...
						CDFvariables[i].putEntry(variableCDFattrs[j], cdfconstant, m.getVariableObject(i).getAttributeObject(variableCDFattrs[j].getName()).value);
					} catch(NullPointerException e){
	...
	/**
	 * @return the string extension of the file that will be created. [".cdf"]
	 */
